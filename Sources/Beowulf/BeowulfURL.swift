/// Beowulf signing URLs.

import Foundation

/// Type representing a Beowulf signing URL

public struct BeowulfURL {
    /// All errors `BeowulfURL` can throw.
    ...
    public static func parse(_ url: URLComponents) throws -> BeowulfURL {
        ...
        return BeowulfURL(type: type, params: params, payload: payload)
    }
    ...
    /// Create a new BeowulfURL from a custom payload.
    ...
    /// Create a new BeowulfURL from a string
    public init?(string: String) {
        guard let url = URLComponents(string: string),
            let _self = try? BeowulfURL.parse(url)
        else {
            return nil
        }
        self = _self
    ...
    /// Create a new BeowulfURL from a URL.
    public init?(url: URL) {
    ...
    /// Create a new BeowulfURL from a set of url components.
    public init?(urlComponents url: URLComponents) {
        guard let _self = try? BeowulfURL.parse(url) else {
            return nil
        }
        self = _self
    }

    /// Create a new BeowulfURL with a transaction, `beowulf://sign/tx/...`
    ...
    /// Create a new BeowulfURL with an operation, `beowulf://sign/op/...`
    ...
    /// Create a new BeowulfURL with several operations, `beowulf://sign/ops/...`
    ...
    public struct ResolveOptions {
        ...
        /// The date string used to fill in the `__createdTime` placeholder.
        public var createdTime: UInt64
        /// Account name used to fill in `__signer` placeholder.
        public var signer: String

        /// Create a new instance.
        public init(refBlockNum: UInt16, refBlockPrefix: UInt32, expiration: Date, createdTime: UInt64, signer: String) {
    ...
    public func resolve(with options: ResolveOptions) throws -> Transaction {
        ...
        func walk(_ value: Any) -> Any {
            ...
            case let str as String:
                ...
                if str == "__createdTime" { return options.createdTime }
